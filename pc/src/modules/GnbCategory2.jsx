...
function GnbCategory2(props) {
  const { className = '' } = props;
  const ref_this = useRef();
  return (
    <>
      <div ref={ref_this} {...props} className={`GnbCategory2 ${className} h-full`}>
            ...
            return (
              <Fragment key={idx1}>
                <li className={`h-full [&:first-child_>_a]:pl-0 [&:last-child_>_a]:pr-0`}>
                  <Link
                    href={`#`}
                    className={`relative block h-full px-[13px] text-[0px] 3xl:px-[15px]`}
                    onMouseEnter={(evt) => {
                    ...
                    <div
                      className={`${
                        bold && `Bold`
                      } Montserrat relative text-[16px] font-[500] text-white hover:underline [&.Bold]:font-[700] [.ChangeBgWrap.On_&]:text-[var(--color1)]`}>
                      {title}
                      {red_dot && (
                        <div
