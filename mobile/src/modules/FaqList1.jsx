...
function FaqList1(props) {
  ...
  return (
    <>
      <div className={`FaqList1 ${className} border-t-[2px] border-t-[var(--color1)]`}>
        {Array.from(Array(5)).map((obj, idx) => {
          return (
            <Fragment key={idx}>
              <button
                className={`grid w-full grid-cols-[33px_1fr] items-start gap-[0] border-b-[1px] border-solid border-b-[var(--color6)] py-[20px] text-left text-[12px] leading-[18px]`}>
                <div className={`font-[700]`}>
                  <span className={`text-[var(--color2)]`}>Q</span>.{idx + 1}
                </div>
                <div>
                  <div className={`Q`}>
                    구매하려는 제품의 색상과 사이즈 <strong className={`font-[700] text-[var(--color2)]`}>교환</strong>
                    이 되지 않습니다.
                  </div>
